...
The disposable demo may use a separate free launcher as a narrow control plane. An authorized visit asks Render to resume the paid PBJ service and opens PBJ after its health check succeeds. While PBJ is running, visible authorized pages report bounded activity; after 10 minutes without that activity, PBJ asks the launcher to suspend it only if there are no active requests, job states, upload inspections/promotions, or FFmpeg/FFprobe processes. Because the free launcher may need time to wake, it obtains a final authenticated no-work confirmation from PBJ immediately before calling Render. Suspension can discard `/tmp` state. The Render API credential is never exposed to PBJ or the browser; it lives only in the launcher's uncommitted `PBJ_RENDER_API_KEY` secret because Render reserves the `RENDER_*` namespace. Because Render API keys are account-scoped rather than service-scoped, this proof remains owner-controlled and the key must be revoked or rotated when the experiment ends.
