...
Provider analysis is not itself a recipe. OpenAI never emits executable shell commands. FFmpeg never decides what is creative or important.
...
- Support safe areas, dynamic viewport changes, reduced motion, resumable uploads, truthful progress, retryable failure states, and Home Screen PWA behavior.
- Private beta may use a server-validated access code remembered for seven days in a secure session.
