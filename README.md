...
The Blueprint expects `PBJ_ACCESS_CODE`, `PBJ_OWNER_CODE`, `OPENAI_API_KEY`, and `TWELVE_LABS_API_KEY` on PBJ plus `RENDER_API_KEY` on the launcher to be entered in Render. `PBJ_SESSION_SECRET`, `PBJ_LAUNCHER_SESSION_SECRET`, and the narrow cross-service control token are generated by Render. Never add these values to Git or `render.yaml`.
