...
For intermittent demonstrations, the Blueprint also defines a small free `pbnj-launcher` service. Open the launcher instead of the paid service: after the private code is accepted, it resumes PBJ and forwards the browser when `/health` succeeds. PBJ requests suspension after 10 minutes with no visible authorized browser activity, but only when no request, upload inspection/promotion, project or recipe job, deletion, FFprobe, or FFmpeg process is active. If the free launcher needs time to wake, it asks PBJ for one final authenticated no-work confirmation immediately before suspending. Background progress polling and Render health checks do not keep it awake. This feature controls compute time; it does not preserve disposable files through suspension.

The launcher requires a Render API key entered only in its `PBJ_RENDER_API_KEY` environment variable (Render reserves the `RENDER_*` namespace). Never commit it, display it in client code, or reuse it elsewhere. Render API keys can reach every workspace available to their owner rather than only PBJ, so use a dedicated key for this proof and revoke or rotate it afterward. The launcher receives PBJ's access code and a generated control token through Blueprint references; the browser never receives either the control token or Render key.
...
The Blueprint expects `PBJ_ACCESS_CODE`, `PBJ_OWNER_CODE`, `OPENAI_API_KEY`, and `TWELVE_LABS_API_KEY` on PBJ plus `PBJ_RENDER_API_KEY` on the launcher to be entered in Render. `PBJ_SESSION_SECRET`, `PBJ_LAUNCHER_SESSION_SECRET`, and the narrow cross-service control token are generated by Render. Never add these values to Git or `render.yaml`.
