...
The `codex/hosted-demo` branch includes a Render Blueprint and Docker image for one private demonstration workspace. Render runs PBJ and FFmpeg independently of the development laptop, and a persistent disk at `/var/data` keeps uploaded projects and completed cuts across deploys.

This mode is deliberately single-instance and access-code protected. Every authorized browser enters the same workspace, so it is appropriate for owner-controlled demonstrations but not for public or multi-user release. Background production still runs in the web process; an interrupted deployment becomes retryable, but durable workers and object storage remain future private-beta work.
